...
export const MessageTypeDefault = withLoggedUser(
    withResponsive(({ usersOnline, viewer, id, user, text, createdAt, files, windowWidth, deleteMessage, updateMessage, isNote, isPin }) => {
        ...
        const handleEdit = useCallback(
            ({ text, fileIds: editedFileIds, mentionedIds }) => {
        ...
        return (
            <>
                <Box
                    ...
                    $bg={isNote ? 'badge-yellow' : isPin ? 'badge-blue' : 'white'}
                    _hover={{
                        $bg: 'bg-gray',
                    }}
                    _active={{
                        $bg: isNote ? 'badge-yellow' : 'bg-light-blue',
                    }}
                    ...
                    $flexDir="column"
                >
                    {isShowControl && !isShowEdit && (
                        <Box $pos="absolute" $zIndex="2" $top="0" $right="0" $transform="translateY(-12px) translateX(-19px)">
                            <Box $d="flex">
                                <DetailActionButton
                                    type="primary"
                                    size="small"
                                    className={isMessageOwner ? 'pin__first' : 'pin__only'}
                                    icon={<IconPin />}
                                    onClick={handlePin}
                                    iscapitalized="true"
                                    $radii={!isMessageOwner ? '10px' : '10px 0 0 10px'}
                                >
                                    {isPin ? 'Unpin' : 'Pin'}
                                </DetailActionButton>
                                {isMessageOwner && (
                    ...
                    {isPin && (
                        <Box $d="flex" $flexDir="row" $alignItems="center" $pb="8">
                            <Box $colorScheme="cta" $mr="8">
                                <IconPin />
                            </Box>
