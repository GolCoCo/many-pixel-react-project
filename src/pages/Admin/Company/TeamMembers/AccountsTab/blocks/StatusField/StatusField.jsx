...
const StatusField = forwardRef(({ value, onChange, handleFieldsChange }, ref) => {
    ...
    return (
        <Box ref={ref} $d="flex" $alignItems="center" $mx="-5">
            ...
            <Text
                ...
                $trans="color 250ms ease-in-out, background 250ms ease-in-out, border-color 250ms ease-in-out"
                $textVariant="Badge"
                $colorScheme={activeStatus === 'PAUSED' ? 'white' : 'primary'}
                $bg={activeStatus === 'PAUSED' ? 'cta' : 'white'}
                $borderW="1"
                $borderStyle="solid"
                $borderColor={activeStatus === 'PAUSED' ? 'cta' : 'outline-gray'}
                $radii="18"
                $px="24"
                $py="6"
                $cursor="pointer"
