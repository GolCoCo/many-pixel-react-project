...
const FormMessageEdit = ({ initialValues, onCancel, onSubmit, onDelete, users }) => {
    ...
    const handleSubmit = () => {
        form.validateFields().then(async values => {
            if (!loading) {
                ...
                let content =
                    textContent.trim() !== ''
                        ? getHtmlFormattedContent(values?.text)
                ...
                if (isMessageNotEmpty || fileIds.length) {
                    ...
                    await onSubmit({ ...values, text: content, fileIds, mentionedIds });
                    setFileIds([]);
                    setHasSent(true);
                    setSubmitting(false);
                } else {
    ...
    return (
        <>
            <Form
                ...
                name="messageEditFormC"
                form={form}
                initialValues={{
                    text: toWysiwyg(initialValues?.text),
                }}
            >
