...
import { COLOR_BADGE_GRAY } from '@components/Theme/color';
import IconPauseRequest from '@components/Svg/IconPauseRequest';
import IconPauseRequestHover from '@components/Svg/IconPauseRequest_hover';
...
import IconDraftRequest from '@components/Svg/IconDraftRequest';
...
const WITH_ICONS = [ORDER_STATUS_ON_HOLD, ORDER_STATUS_DRAFT];
                ...
                {(provided, snapshot) => (
                    ...
                    >
                        {(enableDrag || $isNotCustomer) &&
                            ![ORDER_STATUS_ON_HOLD, ORDER_STATUS_DELIVERED_PROJECT, ORDER_STATUS_COMPLETED, ORDER_STATUS_DRAFT].includes(status) && (
                                <Box
                                    ...
                                    $top="0"
                                    $textAlign="center"
                                    $radii="0px 0px 10px 0px"
                                >
                                    <Text $textVariant="SmallTitle" $colorScheme="cta">
                                        {priority}
                                    </Text>
                                </Box>
                            )}
                        ...
                        <RowItemContainer style={{ cursor: 'default' }} $isOpen={isOpen}>
                            <Box $cursor="default" $h="96" $d="flex" $alignItems="center" $overflow="hidden">
                                {!hasFilter &&
                                    ![ORDER_STATUS_ON_HOLD, ORDER_STATUS_DELIVERED_PROJECT, ORDER_STATUS_COMPLETED, ORDER_STATUS_DRAFT].includes(status) && (
                                ...
                                <Box $w="100%" $minW="0" $maxW="300">
                                    <Box $d="flex" $flexDir="row" $alignItems="center">
                                        {enableDrag && !$isNotCustomer && (
                                            <Box {...provided.dragHandleProps} $pl="16" $pr={['5', '10']} $cursor="move" $w="56" $minW="56">
                                                <IconDragHandler />
                                            </Box>
                                        )}

                                        {hasFilter &&
                                            ![ORDER_STATUS_ON_HOLD, ORDER_STATUS_DELIVERED_PROJECT, ORDER_STATUS_COMPLETED, ORDER_STATUS_DRAFT].includes(
                                                status
                                            ) && <Box $pl="16" $pr={['5', '10']} $cursor="move" $w="56" $minW="56" />}

                                        {(rate && status === ORDER_STATUS_COMPLETED && (
                                            <Box $px="17" $pl="16" $colorScheme={colorRate} $w="56">
                                                <Tooltip
                                            ...
                                            (status === ORDER_STATUS_DELIVERED_PROJECT && (
                                                <Box $px="17" $pl="16" $colorScheme={colorRate} $w="56" onClick={() => setIsShowComplete(true)}>
                                        ...
                                        {!isAdmin && WITH_ICONS.includes(status) && (
                                            <Box $px="7" $pl="16" $colorScheme={colorRate} $w="56">
                                                <Tooltip color="white" title={RATE_MESSAGE[rate]} trigger={['click']}>
                                                    <Box
                                                        as="span"
                                                        $fontSize="22"
                                                        ...
                                                        onClick={() => status !== ORDER_STATUS_DRAFT && setIsShowResume(true)}
                                                    >
                                                        {status === ORDER_STATUS_ON_HOLD && isHovered ? (
                                                            <IconPauseRequestHover />
                                                        ) : (
                                                            status !== ORDER_STATUS_DRAFT && <IconPauseRequest />
                                                        )}
                                                        {status === ORDER_STATUS_DRAFT && <IconDraftRequest />}
                                                    </Box>
                                                </Tooltip>
                                            </Box>
                                        )}
                                        <Box as={Link} to={DETAIL_REQUEST.replace(':id', id)} $px="14" $w="100%">
                                            ...
                                            <Text className="as-link" $d="block" $textVariant="P4" $isTruncate $colorScheme="primary" $fontSize="16" $lineH="28">
                                                <EllipsisMultiple line={2} $lineHeight="20px" $fontSize="14">
                                                    {name}
                                                </EllipsisMultiple>
                                            </Text>
                                        </Box>
                                ...
                                {windowWidth > 768 && (
                                    <>
                                        ...
                                        {!isAdmin && (
                                            <Box $w="100%" $maxW="140" $px="16" $py="12">
                                                {brand && brand.logos?.length > 0 ? (
                                                    <Box $fontSize="10" $lineH="0" $d="inline-block">
                                                        <Avatar src={brandImg} size={24} name={brand?.name} $textVariant="SmallTitle" />
                                                    </Box>
                                                ) : (
                                                    <Box $fontSize="10" $lineH="0" $d="inline-block">
                                                        <IconNoBrand />
                                        ...
                                        {!isAdmin && (
                                            <Box $px="14" $w="100%" $maxW="140" hide="mobile">
                                                {workers.length ? (
                                                    <>
                                                        <Box $d="inline-flex" $justifyContent="center" $alignItems="center">
                                                                    ...
                                                                    <Box $d="flex" $flexDir="column">
                                                                        {workers.map(worker => (
                                                                            <Box
                                                                                ...
                                                                                key={worker.id}
                                                                            >
                                                                                <Avatar src={worker.picture?.url} size={24} name={worker?.name} />
                                                                                <Text $textVariant="P4" $colorScheme="primary" $ml="4">
                                                            ...
                                                            >
                                                                {workers.map((worker, i) => (
                                                                    <Avatar key={`${worker}-${i}`} src={worker.picture?.url} size={24} name={worker?.name} />
                                                                ))}
                                                            </Tooltip>
                                                        </Box>
                                                        <EllipsisMultiple $textVariant="P4" $colorScheme="primary" $textAlign="left" line={1}>
                                                            {workers.map(worker => `${worker.firstname} ${worker.lastname}`).join(', ')}
                                                        </EllipsisMultiple>
                                                    </>
                                                ) : (
                                                    <>
